...
import {
  cloneGridWoErrors,
  setGridErrors,
} from '@/app/puzzle/[id]/components/utils';
...
const PlayingArea: React.FC<{
  ...
  const setGridValue = (num: number | null) =>
    setGrid((oldGrid) => {
      if (
        !oldGrid ||
        !selectedCell ||
        oldGrid[selectedCell.row][selectedCell.col].isPredefined
      ) {
        return oldGrid;
      }
  ...
  const isEditableCellSelected = selectedCell
    ? !grid[selectedCell.row][selectedCell.col].isPredefined
    : false;
