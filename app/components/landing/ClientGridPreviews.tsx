...
const ClientGridPreviews: React.FC<{
  initialGrids: Awaited<ReturnType<typeof getGrids>>;
}> = ({ initialGrids }) => {
  ...
  const onInView = useCallback(
    async (inView: boolean) => {
      ...
      try {
        ...
        const afterParam = grids.length
          ? `&after=${grids[grids.length - 1].id}`
          : '';
        const data = await (
          await fetch(`/apis/grids?limit=${SCROLL_BATCH_SIZE}${afterParam}`)
        ).json();
